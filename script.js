...
// New Year celebration audio
const newYearAudio = new Audio();
newYearAudio.volume = 0.1;
newYearAudio.loop = false;

// Sử dụng file nhạc local
function loadCelebrationMusic() {
	newYearAudio.src = 'motnammoibinhan.mp3';
	newYearAudio.load();
	
	newYearAudio.addEventListener('error', (e) => {
		console.log('Failed to load music:', e);
	}, { once: true });
...
// Load music sau khi DOM ready
if (document.readyState === 'loading') {
	document.addEventListener('DOMContentLoaded', loadCelebrationMusic);
} else {
...
function updateCountdown() {
	...
	// Khi còn 10 giây, tăng độ kịch tính
	const countdownOverlay = document.getElementById('countdownOverlay');
	if (distance < 10000 && distance > 0 && countdownOverlay) {
		countdownOverlay.style.background = 'linear-gradient(135deg, #1a0033, #4d0099, #8000ff)';
		
		// Thêm hiệu ứng shake khi còn 3 giây
		if (distance < 3000) {
			countdownOverlay.style.animation = 'shake 0.5s infinite';
		}
	}
}
...
function triggerNewYearCelebration() {
	...
	// Ẩn countdown với hiệu ứng mượt
	const countdownOverlay = document.getElementById('countdownOverlay');
	if (countdownOverlay) {
	...
	setTimeout(() => {
		...
		// Hiển thị stage container và bắt đầu pháo hoa
		...
		if (typeof store !== 'undefined' && store.setState) {
			store.setState({
				...
				config: {
					...
					skyLighting: 2 // Normal sky lighting
				}
			});
		}
		
		// Tự động enable AudioContext và phát nhạc
		...
		setTimeout(() => {
			try {
				...
				if (playPromise !== undefined) {
					playPromise.catch(e => {
						console.log('Music autoplay blocked, will retry...', e);
					});
				}
			} catch (e) {
		...
		// Hiển thị thông điệp chúc mừng NGAY (0.5s thay vì 2s)
		setTimeout(() => {
			...
			if (message) {
				...
				// Ẩn thông điệp sau 8 giây để tận hưởng pháo hoa
				setTimeout(() => {
		...
		// Bắt đầu finale mode với pháo hoa liên tục
		startFinaleSequence();
...
function startFinaleSequence() {
	// Tạo pháo hoa liên tục trong 60 giây
	let finaleCount = 0;
	const finaleInterval = setInterval(() => {
		if (finaleCount >= 60) {
			clearInterval(finaleInterval);
			// Tiếp tục auto launch sau finale
			return;
		}
...
const store = {
	...
	state: {
		...
		paused: true,
		soundEnabled: true, // Tự động bật sound
		menuOpen: false,
		openHelpTopic: null,
		...
		config: {
			...
			size: IS_DESKTOP
				...
				: IS_HEADER 
					...
					: '2', // Mobile default
			autoLaunch: false, // Tắt autoLaunch, chỉ bật khi countdown xong
			finale: false,
...
function configDidUpdate() {
	...
	if (skyLightingSelector() === SKY_LIGHT_NONE && appNodes.canvasContainer) {
...
if (fullscreenEnabled() && appNodes.fullscreenFormOption) {
	// Fullscreen is supported, show the option
} else if (appNodes.fullscreenFormOption) {
...
function getConfigFromDOM() {
	return {
		quality: appNodes.quality ? appNodes.quality.value : store.state.config.quality,
		shell: appNodes.shellType ? appNodes.shellType.value : store.state.config.shell,
		size: appNodes.shellSize ? appNodes.shellSize.value : store.state.config.size,
		autoLaunch: appNodes.autoLaunch ? appNodes.autoLaunch.checked : store.state.config.autoLaunch,
		finale: appNodes.finaleMode ? appNodes.finaleMode.checked : store.state.config.finale,
		skyLighting: appNodes.skyLighting ? appNodes.skyLighting.value : store.state.config.skyLighting,
		longExposure: appNodes.longExposure ? appNodes.longExposure.checked : store.state.config.longExposure,
		hideControls: appNodes.hideControls ? appNodes.hideControls.checked : store.state.config.hideControls,
		// Store value as number.
		scaleFactor: appNodes.scaleFactor ? parseFloat(appNodes.scaleFactor.value) : store.state.config.scaleFactor
	};
};
...
function init() {
	...
	// Tự động init AudioContext ngay lập tức để nhạc auto-play trên cloud
	soundManager.initAudioContext();
...
const soundManager = {
	...
	ctx: null, // Sẽ được khởi tạo sau khi user interaction
	sources: {
		lift: {
			volume: 0.4,
	...
	// Khởi tạo AudioContext sau user gesture
	initAudioContext() {
		if (!this.ctx) {
			try {
	...
	preload() {
		// Init AudioContext trước khi preload
		this.initAudioContext();
		
		if (!this.ctx) {
			return Promise.resolve(); // Skip nếu không có AudioContext
		}
	...
	playSound(type, scale=1) {
		// Init AudioContext nếu chưa có
		if (!this.ctx) {
			this.initAudioContext();
		}
		
		// Skip nếu vẫn không có ctx hoặc không có buffers
		if (!this.ctx) {
			return;
		}
		...
		if (this.ctx.state === 'suspended') {
			this.ctx.resume().catch(e => console.log('Resume failed:', e));
		}
...
if (IS_HEADER) {
	...
	// Setup user interaction để enable audio
	...
	// Listen cho bất kỳ user interaction nào
...
// ===== HỆ THỐNG HIỂN THỊ LỜI CHÚC TẾT TRÊN PHÁO HOA =====

const newYearWishes = [
	"🎊 Chúc Mừng Năm Mới 2026! 🎊",
	"💰 Phát Tài Phát Lộc 💰",
	"🌟 Vạn Sự Như Ý 🌟",
	"❤️ Sức Khỏe Dồi Dào ❤️",
	"🎯 Chúc Sự Nghiệp Thăng Tiến 🎯",
	"🏡 Ân Khang Thịnh Vượng 🏡",
	"🎉 Năm Mới Vui Vẻ 🎉",
	"✨ May Mắn Rực Rỡ ✨",
	"🌸 Hạnh Phúc Tràn Đầy 🌸",
	"🎁 Tài Lộc Đầy Nhà 🎁",
...
let wishTextElements = [];
const MAX_WISH_TEXTS = 8; // Giới hạn số lời chúc hiển thị cùng lúc

// Tạo container cho wish texts
function createWishTextContainer() {
	let container = document.getElementById('wish-text-container');
	if (!container) {
		container = document.createElement('div');
		container.id = 'wish-text-container';
...
// Hiển thị lời chúc tại vị trí pháo hoa
function showWishText(x, y) {
	// Chỉ hiển thị khi pháo hoa đang active
	if (!isRunning()) return;
	
	// Giới hạn số lượng
	if (wishTextElements.length >= MAX_WISH_TEXTS) {
		const oldest = wishTextElements.shift();
		if (oldest && oldest.parentNode) {
			oldest.parentNode.removeChild(oldest);
		}
	}
	
	const container = createWishTextContainer();
	const wishText = newYearWishes[Math.floor(Math.random() * newYearWishes.length)];
	...
	setTimeout(() => {
		...
		const index = wishTextElements.indexOf(textEl);
		if (index > -1) {
			wishTextElements.splice(index, 1);
		}
	}, 2500);
}

// Inject CSS animation cho wish text
if (!document.getElementById('wish-text-styles')) {
	const style = document.createElement('style');
	style.id = 'wish-text-styles';
	style.textContent = `
...
// Hook vào hàm launch shell để hiển thị wish text
const originalShellLaunch = Shell.prototype.launch;
Shell.prototype.launch = function(position, launchHeight) {
	// Gọi hàm gốc
	originalShellLaunch.call(this, position, launchHeight);
	
	// Hiển thị wish text với xác suất 50% (không phải mọi quả đều có text)
	if (Math.random() < 0.5) {
		// Tính vị trí nổ của pháo hoa
		const width = stageW;
		const height = stageH;
		const hpad = 60;
		const vpad = 50;
		...
		const burstX = position * (width - hpad * 2) + hpad;
		const burstY = minHeight - (launchHeight * (minHeight - vpad));
		
		// Convert to normalized coordinates (0-1)
		const normalizedX = burstX / width;
		const normalizedY = burstY / height;
		
		// Lấy thời gian bay của comet
		const launchDistance = height - burstY;
		const launchVelocity = Math.pow(launchDistance * 0.04, 0.64);
		const hangTime = launchVelocity * (this.horsetail ? 100 : 400);
		
		// Delay để text xuất hiện khi pháo hoa nổ
		setTimeout(() => {
			showWishText(normalizedX, normalizedY);
		}, hangTime * 0.9); // Xuất hiện ngay trước khi nổ
	}
};
